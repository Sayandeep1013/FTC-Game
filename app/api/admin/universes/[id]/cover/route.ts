...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const storagePath = `universe-covers/${slug}-${stamp}.${ext}`;
  ...
  if (uploadError) {
    console.error("Universe cover upload error:", uploadError.message);
  ...
  if (dbError) {
    console.error("Universe cover DB error:", dbError.message);
